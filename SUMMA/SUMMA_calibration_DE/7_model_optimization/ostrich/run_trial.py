...
from utils.ostrich_config import Config # type: ignore
...
#SBATCH --cpus-per-task={config.summa_cpus_per_task}
#SBATCH --time={config.summa_time_limit}
#SBATCH --mem={config.summa_mem}
...
gru_max={config.summa_gru_count}
gru_count={config.summa_gru_per_job}
...
summa_command="{config.summa_exe} -g $gru_start $gru_count -m {config.summa_filemanager} --caf.scheduler.max-threads=$SLURM_CPUS_PER_TASK"
$summa_command > {config.summa_log_path}/summa_log_rank{rank}_$SLURM_ARRAY_TASK_ID.txt
"""
...
def post_process_summa_output(config, rank):
    ...
    summa_output_path = get_rank_specific_path(Path(config.root_path) / f"domain_{config.domain_name}", 
                                               config.experiment_id, "SUMMA", rank) / "output"
    mizuRoute_input_path = get_rank_specific_path(Path(config.root_path) / f"domain_{config.domain_name}", 
                                                  config.experiment_id, "mizuRoute", rank) / "input"

    src_dir = Path(summa_output_path)
    src_pat = f"{config.experiment_id}_day.nc"
    src_var = "averageRoutedRunoff"  # Ensure this is the correct variable name
    des_dir = Path(mizuRoute_input_path)
    des_fil = f"{config.experiment_id}_{{}}_.nc"

    # Extract start and end years from config.calib_period and config.eval_period
    start_year = min(config.calib_period[0].year, config.eval_period[0].year)
    end_year = max(config.calib_period[1].year, config.eval_period[1].year)
...
def run_summa(config, rank):
    if config.use_parallel_summa:
        ...
        summa_exe = f'{config.root_path}/installs/summa/bin/{config.exe_name_summa}'
        file_manager = get_summa_settings_path(config, rank) / config.settings_summa_filemanager
...
def run_mizuroute(config, rank):
    mizuroute_exe = f'{config.root_path}/installs/mizuroute/route/bin/{config.exe_name_mizuroute}'
    control_file = get_mizuroute_settings_path(config, rank) / config.mizu_control_file
...
def get_summa_settings_path(config, rank):
    rank_specific_path = get_rank_specific_path(Path(config.root_path) / f"domain_{config.domain_name}", 
                                                config.experiment_id, "SUMMA", rank)
    return rank_specific_path / "run_settings"

def get_mizuroute_settings_path(config, rank):
    rank_specific_path = get_rank_specific_path(Path(config.root_path) / f"domain_{config.domain_name}", 
                                                config.experiment_id, "mizuRoute", rank)
    return rank_specific_path / "run_settings"
...
def calculate_objective_function(config, rank):
    ...
    try:
        ...
        return [float('inf')] * len(config.optimization_metrics)
...
def evaluate_model(config, rank):
    ...
    try:
        ...
        segment_index = sim_data['reachID'].values == int(config.sim_reach_ID)
        sim_df = sim_data.sel(seg=segment_index)['IRFroutedRunoff'].to_dataframe().reset_index()
        sim_df.set_index('time', inplace=True)

        # Read observation data
        obs_df = pd.read_csv(config.obs_file_path, index_col='datetime', parse_dates=True)
        obs_df = obs_df['discharge_cms'].resample('h').mean()

        print(obs_df, 'observations')
        print(sim_df, 'simulations')


        # Calculate metrics for calibration period
        calib_metrics = calculate_metrics(
            obs_df.loc[config.calib_period[0]:config.calib_period[1]],
            sim_df.loc[config.calib_period[0]:config.calib_period[1]]['IRFroutedRunoff']
        )

        # Calculate metrics for evaluation period
        eval_metrics = calculate_metrics(
            obs_df.loc[config.eval_period[0]:config.eval_period[1]],
            sim_df.loc[config.eval_period[0]:config.eval_period[1]]['IRFroutedRunoff']
        )
...
def get_mizuroute_output_path(config, rank):
    ...
    output_file = output_dir / f"{config.experiment_id}_rank{rank}.h.*.nc"  # Adjust the file pattern as needed
...
def update_trial_params(config, rank):
    ...
    all_params = config.params_to_calibrate + config.basin_params_to_calibrate
    ...
    trial_param_file = summa_settings_path / config.trial_param_file
...
def main():
    try:
        ...
        logger.error(f"An error occurred in run_trial.py: {str(e)}")
        logger.error(traceback.format_exc())
        sys.exit(1)
