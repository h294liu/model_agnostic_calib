...
from utils.ostrich_config import Config # type: ignore  # type: ignore 
...
class OstrichOptimizer:
    def __init__(self, config: Config):
        ...
        self.logger = get_logger(f'OstrichOptimizer_rank{self.rank}', config.root_path, config.domain_name, config.experiment_id)
        self.num_ranks = self.config.num_ranks #int(os.environ.get('PMI_SIZE', 1))  # Get total number of ranks
    ...
    def create_rank_specific_directories(self):
        for rank in range(1, self.num_ranks):
            for model in ["SUMMA", "mizuRoute"]:
                rank_specific_path = Path(f'{self.config.root_path}/domain_{self.config.domain_name}/simulations/{self.config.experiment_id}_rank{rank}/{model}')
                ...
                settings_summa_path = Path(f'{self.config.root_path}/domain_{self.config.domain_name}/settings/SUMMA')
                settings_mizu_path = Path(f'{self.config.root_path}/domain_{self.config.domain_name}/settings/mizuroute')
...
BeginParallelStuff
ParallelType   MPI
NumNodes       {self.config.num_ranks}
EndParallelStuff
...
"""
    ...
    def generate_observations(self):
        if len(self.config.optimization_metrics) == 1:
            ...
            return "\n".join([f"Obj{i+1} 1 1 ostrich_objective.txt; OST_NULL 0 1" for i in range(len(self.config.optimization_metrics))])

    def generate_response_vars(self):
        if len(self.config.optimization_metrics) == 1:
            return f"Obj ostrich_objective.txt; OST_NULL 0 1"
        else:
            return "\n".join([f"Obj{i+1} ostrich_objective.txt; OST_NULL 0 1" for i in range(len(self.config.optimization_metrics))])
        
    def generate_obj_function(self):
        return "\n".join([f"ObjFunc Obj{i+1}" for i in range(len(self.config.optimization_metrics))])
    ...
    def generate_param_definitions(self):
        definitions = []
        all_params = self.config.params_to_calibrate + self.config.basin_params_to_calibrate
        all_bounds = self.config.all_bounds  # Make sure this includes bounds for both local and basin parameters
    ...
    def generate_algorithm_config(self):
        algorithm = self.config.ostrich_algorithm
        config = ""

        if algorithm == "DDS":
            config = f"""
BeginDDSAlg
PerturbationValue   {self.config.dds_r}
...
BeginParaPADDS
PerturbationValue  {self.config.dds_r}
PopulationSize     {self.config.pop_size}
NumGenerations     {self.config.nsga2_n_gen}
ArchiveSize        100
UpdateInterval     10
MaxIterations      1000
EndParaPADDS
"""
    ...
    def get_summa_settings_path(self, rank):
        ...
        rank_specific_path = self.get_rank_specific_path(Path(self.config.root_path) / f"domain_{self.config.domain_name}", 
                                                        self.config.experiment_id, "SUMMA", rank)
    ...
    def copy_summa_settings(self, source_path, destination_path, rank):
        files_to_copy = [
            'outputControl.txt', self.config.settings_summa_filemanager, 'attributes.nc', 
        ...
        self.update_summa_file_manager(destination_path / self.config.settings_summa_filemanager, rank)

    def copy_mizuroute_settings(self, source_path, destination_path, rank):
        files_to_copy = [self.config.mizu_control_file, 'param.nml.default', 'topology.nc']
        for file in files_to_copy:
            shutil.copy2(source_path / file, destination_path / file)
        
        # Update control file for the specific rank
        self.update_mizu_control_file(destination_path / self.config.mizu_control_file, rank)

    def update_summa_file_manager(self, file_path, rank):
        with open(file_path, 'r') as f:
            content = f.read()
        
        content = content.replace(self.config.experiment_id, f'{self.config.experiment_id}_rank{rank}')
    ...
    def reset_file_manager(self, settings_path, rank):
        ...
        sim_start_time = (self.config.calib_period[0] - relativedelta(years=1))
        sim_end_time = self.config.eval_period[1] + datetime.timedelta(hours=23)
    ...
    def run_summa_for_priori(self, rank):
        ...
        file_manager = self.get_summa_settings_path(rank) / self.config.settings_summa_filemanager
        
        # Create SUMMA_logs directory in the rank-specific path
        log_path = self.get_rank_specific_path(
            Path(self.config.root_path) / f"domain_{self.config.domain_name}",
    ...
    def create_priori_trial_param_file(self, settings_path, rank):
        ...
        rank_experiment_id = f"{self.config.experiment_id}_rank{rank}"
