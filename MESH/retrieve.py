...
end_date = "2016-01-01"
catchment_shapefile = '../catchment/4815_catchments.shp' #from geofabric

#obtain stations for the specified province
stations = get_stations(province, collection_stations, api_url)

#create a list with stations id only
stations_id = [el[0] for el in stations]

#retrieve data for the list of stations from the province and removes stations without data
...
#add coordinates to the list of stations with data
...
#temporary 
#stations_df = pd.read_csv('stations_with_data_2013.csv')

#create geopandas dataset with the stations with data and obtain the subbasin id for each station
...
stations_gdf['COMID'] = ""
for el in range(len(catchment)):
	for pt in range(len(stations_gdf)):
		if catchment['geometry'][el].contains(stations_gdf['geometry'][pt]):
			stations_gdf.loc[pt,'COMID'] = catchment.loc[el,'COMID']

#remove stations that are not located within a polygon (e.g. outside of area of interest)
clean_list = stations_gdf[stations_gdf['COMID'].astype(bool)]

#save stations to csv including coordinates
clean_list[['ID','COMID',]].reset_index(drop=True).to_csv('./stationsMapping.csv')
